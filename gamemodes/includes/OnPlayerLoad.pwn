...
public OnPlayerLoad(playerid)
{
	...
	if (PlayerInfo[playerid][pAdmin] > 0)
	{
		...
		else
		{
			...
			format(string, sizeof(string), "SERVER: Tai khoan dang nhap la %s{FFFF66}.", GetStaffRank(playerid));
			SendClientMessageEx(playerid, COLOR_WHITE, string);
			format(string, sizeof(string), "SERVER: %s da dang nhap %s{FFFF66}.", GetPlayerNameEx(playerid), GetStaffRank(playerid));
		}
	...
	printf("%s Dang Nhap Thanh Cong.", GetPlayerNameEx(playerid));
	format(string, sizeof(string), "{003366}Welcome May Chu Gta San Mobile Viet Nam");
	SendClientMessageEx(playerid, COLOR_WHITE, string);
