...
{
	//Prefix_OnrDynamicArea(playerid, areaid);
	if(areaid == NGGShop)
	{
...
/*	PlayerTextDrawShow(playerid, kontenTD[playerid]);
	...
	if(PlayerInfo[playerid][pReal] >= 3 && GetPVarInt(playerid, "Injured") == 1)
	{
		if(PlayerInfo[playerid][pChetne] == 0)
		{
		    new string[128];
		    TogglePlayerControllable(playerid, 0);
		    ClearAnimations(playerid);
            SetPlayerHealth(playerid, 200);
            format(string, sizeof(string), "(( %s da chet. ))", GetPlayerNameEx(playerid));
	        ...
	        GameTextForPlayer(playerid, "~r~BAN DA CHET /chapnhan chet", 3000, 3);
	        SendClientMessage(playerid, COLOR_RED, "[!] Ban da chet");
	        PlayerInfo[playerid][pReal] = 0;
	        PlayerInfo[playerid][pChetne] = 1;
	    }
    }

	if(PlayerInfo[playerid][pOLD] >= 1)
	{
	new string[200];
    format(string, sizeof(string), "Thanh Vien Lau Nam (GvR)");
	SetPlayerChatBubble(playerid,string,COLOR_DBLUE,50.0,10000);
	}
	if(PlayerInfo[playerid][pLevel] < 2)
	{
	new string[200];
    format(string, sizeof(string), "Newbie Vui Long Khong Tan Cong");
	SetPlayerChatBubble(playerid,string,COLOR_LIGHTBLUE,50.0,10000);
	}
	// toc do xe
	...
	if(!FindTimePoints[playerid] && arr_Engine{GetPlayerVehicleID(playerid)} != 0) {
		...
		format(szSpeed, sizeof(szSpeed),"~n~~n~~n~~n~~n~~n~~n~~n~~n~~w~%.0f Toc Do MPH", player_get_speed(playerid));
		GameTextForPlayer(playerid, szSpeed, 250, 3);
		}
	}*/
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid)
{
	...
	if(InfoMessage[playerid] == true) // version 2
	{
       if(issuerid != INVALID_PLAYER_ID)
       {
        ...
        format(infoString, sizeof(infoString), "[Damage] Ban bi {33FFCC}%s{ffffff} su dung {9900CC}%s{ffffff} gay ton that {ff0033}%.02f", attackerName, weaponName, amount);
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid)
{
    ...
    GetWeaponName(weaponid, weaponname, sizeof (weaponname));
    format(string, sizeof(string), "[Damage] Ban da dung {9900CC}%s{ffffff} gay cho {008800}%s{ffffff} ton that {ff0033}%.02f", weaponname, victim, amount);
...
new VehicleDebug[MAX_PLAYERS];
new Float:MyPos[MAX_PLAYERS][3];

task CheckEnginePlayer[2000] () {
	for(new i = 0; i < GetMaxPlayers(); i ++) {
		if(IsPlayerInVehicle(i, VehicleDebug[i]) && GetPlayerState(i) == PLAYER_STATE_DRIVER) { // Dieu kien nguoi choi co tren xe hay khong
			if(!IsPlayerInRangeOfPoint(i, 2, MyPos[i][0],MyPos[i][1],MyPos[i][2])) { // Check khoi dong xe
				new engine,lights,alarm,doors,bonnet,boot,objective;
			    GetVehicleParamsEx(VehicleDebug[i],engine,lights,alarm,doors,bonnet,boot,objective); // Kiem tra du lieu cua engine co duoc bat hay khong
			    if(engine == VEHICLE_PARAMS_OFF) { // Eng ko duoc bat 
			    	TogglePlayerControllable(i, 0); // Freeze
			    	SetTimerEx("FreezePlayer_F",5000,0,"d",i);
			    	SendClientMessage(i, -1, "Ban can khoi dong xe!");
				}
				else GetPlayerPos(i,MyPos[i][0],MyPos[i][1],MyPos[i][2]);
			}
...
forward FreezePlayer_F(playerid);
public FreezePlayer_F(playerid) {
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	VehicleDebug[playerid] = vehicleid;
	...
	if(GetPVarType(playerid, "GiaoHang1") && !(IsAGiaoHang1Car(vehicleid)))
	{
	    new Float:slx, Float:sly, Float:slz;
		GetPlayerPos(playerid, slx, sly, slz);
		SetPlayerPos(playerid, slx, sly, slz+1.3);
		...
		RemovePlayerFromVehicle(playerid);
		defer NOPCheck(playerid);
		SendClientMessageEx(playerid, COLOR_GRAD2, "ban can o vi tri lay hang");
		return 1;
	}
	if(!ispassenger)
	{
		...
		else if(IsAGiaoHang1Car(vehicleid))
		{
		    if(PlayerInfo[playerid][pJob] != 22 && PlayerInfo[playerid][pJob2] != 22)
		    {
			    ...
			    RemovePlayerFromVehicle(playerid);
			    defer NOPCheck(playerid);
			    SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong phai nhan vien giao hang");
			}
		}
...
public OnPlayerConnect(playerid) {
	...
	GetPlayerIp(playerid, ip, sizeof(ip));
	format(sting, sizeof(sting), "[Kiem Soat]{006666}%s Da Tham Gia may chu - Dia chi IP:[%s]", name, ip);
	foreach(new i: Player)
	{
    	if(PlayerInfo[i][pAdmin] >= 2)
    		{
			SendClientMessage(i,COLOR_GREY, sting);
    		}
	}
	...
	switch(reason)
	{
		case 0:
		{
			...
			if(PlayerCuffed[playerid] != 0)
			{
				...
				PlayerInfo[playerid][pJailTime] += 30*30;
				PlayerInfo[playerid][pCansa] = 0;
				PlayerInfo[playerid][pPot] = 0;
				PlayerInfo[playerid][pCrack] = 0;
				PlayerInfo[playerid][pTuicansa] = 0;
			}
		}
		case 1:
		{
...
{
	if(IsAMedic(playerid) && PlayerInfo[playerid][pDuty]) {
		GetPVarInt(playerid, "IsInArena");
		PlayerInfo[killerid][pWantedLevel] += 2;
		SetPlayerWantedLevel(killerid, PlayerInfo[killerid][pWantedLevel]);
		SendClientMessageEx (killerid, COLOR_RED, "Ban Da Giet Mot Bac Si Dang Tren Duong Cap Cuu Va Ban Bi Truy Na 2 Sao!");
	}

	if(IsACop(playerid) && PlayerInfo[playerid][pDuty]) {
		GetPVarInt(playerid, "IsInArena");
		PlayerInfo[killerid][pWantedLevel] += 2;
		SetPlayerWantedLevel(killerid, PlayerInfo[killerid][pWantedLevel]);
		SendClientMessage (killerid, COLOR_RED, "Ban Da Giet Mot Si Quan Canh Sat Va Ban Bi Truy Na 2 Sao!");
	}
	...
	if(GetPVarInt(playerid, "EventToken") == 0)
	{
	    if(GetPVarInt(playerid, "IsInArena") == -1)
		{
				...
				SetPVarFloat(playerid, "MedicX", mX);
				SetPVarInt(playerid, "ThoiGianChet", 180); // So giay dat tai day
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	for(new h = 0; h < MAX_POINTS; h++)
	{
		...
		else if(GetPVarInt(playerid, "MatDeliver") == 444 && IsPlayerInRangeOfPoint(playerid, 6.0, 314.7733,1151.8701,8.5859))
		{
			if(GetPVarInt(playerid, "Packages") > 0)
			{
				if(PlayerInfo[playerid][pDonateRank] == 1)
	...
	if(gPlayerCheckpointStatus[playerid] == CHECKPOINT_DELIVERY)
	{
		...
		gPlayerCheckpointStatus[playerid] = CHECKPOINT_RETURNTRUCK;
		SetPlayerCheckpoint(playerid, 8.9755,1383.6484,9.1781, 5);
		GameTextForPlayer(playerid, "~w~Dia diem ~r~Ben cang San Fierro", 5000, 1);
		SendClientMessageEx(playerid, COLOR_WHITE, "HINT: Quay l cang ai benSan Fierro (diem checkpoint tren radar).");
	...
	if(GetPVarInt(playerid, "Pizza") > 0)
	{
		...
		new sotien = 3000;
		format(string, sizeof(string), "Ban da van chuyen banh pizza toi dich! Ban nhan duoc $%d tien banh", sotien+rand);
		Misc_Save();
		GivePlayerCash(playerid, sotien+rand);
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			case CHECKPOINT_RETURNTRUCK:
			{
				...
				{
				    distancepay = floatmul(GetDistance(8.9755,1383.6484,9.1781, TruckerDropoffs[route][PosX], TruckerDropoffs[route][PosY], TruckerDropoffs[route][PosZ]), 1.5);
				}
				...
				if(TruckDeliveringTo[vehicleid] != INVALID_BUSINESS_ID) {
					...
					if (payment > 6500) payment = 10000;

					GivePlayerCash(playerid, payment); 
					PlayerInfo[playerid][pPhieuThuong] += 4; // +1 phieu thuong
					format(string, sizeof(string), "*. Ban duoc tra $%d cho viec van chuyen hang hoa va tra lai xe.", payment);
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
					format(string, sizeof(string), "[Phan Thuong] Nhan Them Duoc 4 Phieu Thuong", payment);
				    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
				    SetVehicleToRespawn(vehicleid);
				    print("deptrathicogilasainhungmaloikhongfixduoc");
					...
					if(truckdeliver == 5) // Weapons
					{
						if(PlayerInfo[playerid][pConnectHours] >= 2 && PlayerInfo[playerid][pWRestricted] <= 0)
						{
							...
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* You were also given a free bat as a bonus for taking the risk of transporting illegal weapons.");
								GivePlayerValidWeapon(playerid, 5, 1);
							}
							else if(level >= 151 && level <= 300)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* You were also given a free Katana as a bonus for taking the risk of transporting illegal weapons.");
								GivePlayerValidWeapon(playerid, 8, 60000);
							}
							else if(level >= 300 && level <= 450)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* You were also given a free GOLFCLUB as a bonus for taking the risk of transporting illegal weapons.");
								GivePlayerValidWeapon(playerid, 2, 60000);
							}
							else if(level >= 450 && level <= 550)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* You were also given a free SHOVEL as a bonus for taking the risk of transporting illegal weapons.");
								GivePlayerValidWeapon(playerid, 6, 50);
							}
							else if(level >= 700)
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* You were also given a free Sex toy as a bonus for taking the risk of transporting illegal weapons.");
								GivePlayerValidWeapon(playerid, 10, 100);
							}
						}
						else
						{
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(newkeys % KEY_YES)
	{
	if(IsPlayerInRangeOfPoint(playerid, 3.0, -1472.3840,-1780.6263,3288.7859) && (PlayerInfo[playerid][pDuocCapGiay] >= 1))
	ShowPlayerDialog(playerid, DIALOG_CAPGIAYPHEP,  DIALOG_STYLE_LIST, "Ho So","Nhan Giay Phep", "O", "X");
	}
	...
	if(newkeys & KEY_YES)
	{
	if(IsPlayerInRangeOfPoint(playerid, 1.0, -55.8932,1185.4470,19.3917) && (PlayerInfo[playerid][pGiayPhepVuKhi] >= 1))
	ShowPlayerDialog(playerid, DIALOG_CHETAOVUKHI3,  DIALOG_STYLE_LIST, "Ban Muon che tao vu khi nao","Sung Luc\nShotgun\nSung Truong", "O", "X");
	}
	if (newkeys & KEY_YES)
	{
	cmd_vao(playerid, "");
	...
	if(GetPlayerState(playerid) == 1 && PlayerInfo[playerid][pJob] == 0 || PlayerInfo[playerid][pJob2] == 0) {
		if(IsPlayerInRangeOfPoint(playerid,3.0,251.99, 117.36, 1003.22) || IsPlayerInRangeOfPoint(playerid,3.0,301.042633, 178.700408, 1007.171875) || IsPlayerInRangeOfPoint(playerid,3.0,-1385.6786,2625.6636,55.5572)) {
			...
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
			...
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 2;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
		...
		else if (IsPlayerInRangeOfPoint(playerid,3.0,-167.9084,1188.7351,19.7422)) {
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 3;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
			...
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 4;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
		...
		else if (IsPlayerInRangeOfPoint(playerid,3.0,-94.9299,1108.6356,19.7422) || IsPlayerInRangeOfPoint(playerid,3.0,-1425.0492,2591.6218,55.8230) || IsPlayerInRangeOfPoint(playerid,3.0,-1475.4224,1877.3550,32.6328)) {
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 7;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
		...
		else if (IsPlayerInRangeOfPoint(playerid,3.0,1224.13, 267.98, 19.55) || IsPlayerInRangeOfPoint(playerid,3.0,-105.8922,1185.7015,19.7422) || IsPlayerInRangeOfPoint(playerid,3.0,2226.1716,-1718.1792,13.5165) || IsPlayerInRangeOfPoint(playerid,3.0,1099.73,-1504.67,15.800) || IsPlayerInRangeOfPoint(playerid,3.0,-821.3508,1574.9393,27.1172)) {
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 8;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
			...
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 9;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
		...
		/*else if (PlayerInfo[playerid][pJob] == 0 && GetPlayerState(playerid) == 1 && IsPlayerInRangeOfPoint(playerid,3.0,531.7930,-1292.4044,17.2422)) {
			...
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 12;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
			...
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 14;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
		...
		/*else if (PlayerInfo[playerid][pJob] == 0 && GetPlayerState(playerid) == 1 && IsPlayerInRangeOfPoint(playerid,3.0,-2040.9436,456.2395,35.1719)) {
			...
			return 1;
				...
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");			GettingJob[playerid] = 16;
				return 1;
		}
			...
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 17;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
		...
		else if (IsPlayerInRangeOfPoint(playerid,3.0,-123.4430,1081.2023,19.7742) || IsPlayerInRangeOfPoint(playerid,3.0,-1512.2278,2611.8994,55.7666)) {
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 18;
				return 1;
			}
			if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
			...
			if(PlayerInfo[playerid][pJob] == 0){
				SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Neu ban da chac chan lua chon cong viec Nguoi pha che, su dung /chapnhan congviec de hoan tat.");
			...
			{
				if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");				GettingJob[playerid] = 20;
				return 1;
				}
				if((PlayerInfo[playerid][pDonateRank] > 0 || PlayerInfo[playerid][pFamed] > 0) && PlayerInfo[playerid][pJob2] == 0) {
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");					SendClientMessageEx(playerid, COLOR_YELLOW, "Ban nhan them cong viec phu. Chi VIP/Famed moi  lam duoc dieu nay");
			...
			else return SendClientMessageEx(playerid, COLOR_GREY, "Ban phai dat it nhat level 2 moi co the tro thanh Nguoi dua hang.");
		}
		else if (IsPlayerInRangeOfPoint(playerid,3.0,-1718.90,1355.63,7.18)) {
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");
		...
		else if (IsPlayerInRangeOfPoint(playerid,3.0,-1316.7323,2700.7727,50.2663)) {
			if(PlayerInfo[playerid][pJob] == 0){
				ShowPlayerDialog(playerid, DIALOG_XINVIEC, DIALOG_STYLE_MSGBOX, "Nhan Viec", "Ban Co Muon Nhan Cong Viec Nay Khong", "O", "X");
    ...
    if(newkeys & KEY_NO)
    {
        if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
       {
            ...
            {
            new string[128];
            format(string, sizeof(string), "{FF8000}** {99FF33}%s [Phuong Tien]{C2A2DA} dua chia khoa vao o va bat dong co xe.", GetPlayerNameEx(playerid));
            ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
...
/*    if(masked[playerid])
	...
	switch(PlayerInfo[playerid][pAccent])
	{
		...
		case 2: accent = "{E068E6}(Mien Tay){FFFFFF} ";
		case 3: accent = "{F03535}(Sai Gon){FFFFFF} ";
		case 4: accent = "{7DD183}(Ca Mau){FFFFFF} ";
		...
		case 27: accent = "{ffc0cb}(Con Gai){FFFFFF} ";
		case 28: accent = "{7fff00}(Con Trai){FFFFFF} ";
		case 29: accent = "{ff0000}(Administrator){FFFFFF} ";
	}
