...
*/
    ...
    format(string128,sizeof(string128),"{FFFF00}Luu y: %s (ID: %i) vua dang ky tham gia may chu", name, ip);
    format(string128,sizeof(string128),"{007700}Hay Kiem Tra /whitelist va duyet");
...
/*	if(PlayerInfo[playerid][pXacNhanDangKy] == 0) { // Kiem tra xac nhan
	...
	for(new x;x<10000;x++)
	{
		...
		//ShowPlayerDialog(playerid, CHAOMUNGDENVOISERVER, DIALOG_STYLE_MSGBOX, "Chao mung ban den voi Server", "Duoi day la mot so nhung lenh co ban khi moi bat dau tham gia server GvR Roleplay:\n+ /newb neu ban muon hoi mot dieu gi day trong game\n+ /yeucautrogiup de nho doi ngu Helper ho tro ban ve cach choi\n+ /baocao neu ban gap loi, su co hoac la gap hacker/cheater", "OK", "");
	SendClientMessage(playerid, -1, "{ffffff}=============== {ffd905}SA-MP Mobile{ffffff} ===============");
	SendClientMessage(playerid, -1, "{ffd905}C{ffffff}hao mung ban da den voi GvR");
	SendClientMessage(playerid, -1, "{ffffff}Thanh Pho San Fierro");
	SendClientMessage(playerid, -1, "{ffd905}hay /timvieclam De Tim Mot Cong Viec Phu Hop Cho Minh");
	SendClientMessage(playerid, -1, "{ffffff}Ban vua dang ky thanh cong hay den tru so canh sat");
	SendClientMessage(playerid, -1, "{ffffff}Lam Ho So Can Cuoc De Co The Bat Dau Cong Viec");
	SendClientMessage(playerid, -1, "{ffffff}Ban hay nhan mot cong viec sau do su dung /telejob nhe");
	SendClientMessage(playerid, -1, "{ffffff}Ban co 5 luot Tele Job Hay su dung /telejob den cong viec ma ban mong muon");
	SendClientMessage(playerid, -1, "{ffffff}Phien ban:"SERVER_GM_TEXT"");
	SendClientMessage(playerid, -1, "{ffffff}==========================================");
	SetCameraBehindPlayer(playerid);
	SetPlayerVirtualWorld(playerid, 0);
	PlayerInfo[playerid][pTeleJob] += 5;
	PlayerInfo[playerid][pRadioFreq] = 1;
	PlayerInfo[playerid][pRadio] = 1;
	SetPlayerPos(playerid, -1724.6941,1352.7980,7.1875);
    return 1;
...
{
	printf("warning fix: newweapon %d", newweapon);
...
public InitiateGamemode()
{
	...
	print("\n-------------------------------------------");
	print("Cong Dong GTA Viet Nam (Mobile)\n");
	print("Copyright (C) GvR Group, LLC (2015-2100)");
...
public SetVehicleEngine(vehicleid, playerid)
{
    ...
    if(engine == VEHICLE_PARAMS_ON)
	{
		...
		format(string, sizeof(string), "{FF8000}**{99FF33} %s [Phuong Tien]{C2A2DA} da tat dong co phuong tien.", GetPlayerNameEx(playerid));
    ...
    else if(engine == VEHICLE_PARAMS_OFF || engine == VEHICLE_PARAMS_UNSET)
	{
		...
		else SendClientMessageEx(playerid, COLOR_WHITE, "Phuong tien da khoi dong thanh cong ({FF0000}/xe khoidong{FFFFFF} de tat phuong tien).");
	...
	switch(GetPVarInt(iPlayer, "StreamPrep")) {
		...
		default: {
			...
			if(GetPVarType(iPlayer, "MedicCall")) {
				...
				ApplyAnimation(iPlayer, "PED", "KO_shot_stom", 4.0, 0, 1, 1, 1, 0, 1);
			}
...
public Maintenance()
{
	...
	SendRconCommand("hostname GvR-Cong dong GTA Viet Roleplay (Mobile) [Khoi dong lai de Bao tri]");
	foreach(new i: Player)
	{
		if(gPlayerLogged{i}) {
	...
	if(GetPVarInt(playerid, "HelpTime") > 0)
 	{
    	...
    	if(GetPVarInt(playerid, "HelpTime") == 0)
     	{
      		SendClientMessageEx(playerid, COLOR_GREY, "Yeu cau tro giup cua ban da het han. Neu day la mot loi hay tham gia discord GvR de bao loi.");
...
stock SetPlayerJoinCamera(playerid)
{
	...
	switch(randcamera)
	{
		case 1: // Gym
		{
			...
			SetPlayerInterior(playerid, 0);
			Streamer_UpdateEx(playerid,-1515.3134,731.1772,23.4790);
			SetPlayerPos(playerid,-1515.3134,731.1772,-10.0);
			SetPlayerCameraPos(playerid,1514.7783,-1700.2913,36.7506);
			SetPlayerCameraLookAt(playerid,-1515.3134,731.1772,23.4790);
		}
...
stock ShowMainMenuDialog(playerid, frame)
{
	...
	switch(frame)
	{
		case 1:
		{
			format(titlestring, sizeof(titlestring), "{FFFF66}Dang nhap - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "Welcome GvR (Mobile)\n{FFFFFF}Nhap Mat Khau:\nDia Chi IP:%s", GetPlayerIpEx(playerid));
			ShowPlayerDialog(playerid,MAINMENU,DIALOG_STYLE_PASSWORD,titlestring,string,"Dang nhap","Thoat");
		}
		case 2:
		{
			format(titlestring, sizeof(titlestring), "{FFFF66}Dang ky - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "{FFFFFF}Chao mung ban den voi Cong dong GTA Viet Roleplay (Moible), %s.\n\nDia chi IP: %s\n\nNhan vat %s chua ton tai, vui long dien mat khau de tao nhan vat:", GetPlayerNameEx(playerid), GetPlayerIpEx(playerid), GetPlayerNameEx(playerid));
			ShowPlayerDialog(playerid,MAINMENU2,DIALOG_STYLE_INPUT,titlestring,string,"Dang ky","Thoat");
		}
		case 3:
		{
			format(titlestring, sizeof(titlestring), "{FFFF66}Dang nhap - %s", GetPlayerNameEx(playerid));
			format(string, sizeof(string), "{FF0000}Mat khau khong hop le!\nTai khoan %s ma ban dang su dung da duoc dang ky, nhap mat khau de dang nhap:", GetPlayerNameEx(playerid));
			ShowPlayerDialog(playerid,MAINMENU,DIALOG_STYLE_PASSWORD,titlestring,string,"Dang nhap","Thoat");
		}
		case 4:
		{
			format(titlestring, sizeof(titlestring), "{FFFF66}Tai khoan dang tham gia - %s", GetPlayerNameEx(playerid));
...
stock GetJobName(job)
{
	...
	switch(job)
	{
		...
		case 22: name = "Cong ty van chuyen";
...
stock GetJobLevel(playerid, job)
{
	...
	switch(job)
	{
		...
		case 20:
		{
			...
			if(skilllevel >= 0 && skilllevel <= 151) jlevel = 1;
			else if(skilllevel >= 151 && skilllevel <= 300) jlevel = 2;
			else if(skilllevel >= 300 && skilllevel <= 450) jlevel = 3;
			else if(skilllevel >= 450 && skilllevel <= 550) jlevel = 4;
	...
	if(GetPVarInt(playerid, "MedicBill") == 1 && PlayerInfo[playerid][pJailTime] == 0)
	{
		switch(PlayerInfo[playerid][pHospital]) {
			case 1:
			{
				...
				SetPlayerPos(playerid, -317.8576,1058.7970,19.7422);
			...
			case 10:
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien $250. Chuc mot ngay tot lanh!");
				SetPlayerPos(playerid, -2657.4409,625.6119,14.4531);
...
stock IsAtFishPlace(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		if(IsPlayerInRangeOfPoint(playerid,1.0,-1483.7213,742.8031,7.1778) || IsPlayerInRangeOfPoint(playerid,1.0,-1483.7202,745.3783,7.1778))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,1.0,-1483.7202,747.6473,7.1778) || IsPlayerInRangeOfPoint(playerid,1.0,-1483.7203,752.1034,7.1778))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,1.0,-1483.7195,760.3973,7.1778) || IsPlayerInRangeOfPoint(playerid,1.0,-1483.7203,752.1034,7.1778))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,1.0,-1483.7194,764.5928,7.1778) || IsPlayerInRangeOfPoint(playerid,1.0,-1483.7208,768.5953,7.1778))
		{//Fishplace at the bigwheel
			return 1;
		}
		else if(IsPlayerInRangeOfPoint(playerid,1.0,-1483.7196,776.7362,7.1778) || IsPlayerInRangeOfPoint(playerid,1.0,-1483.7222,773.2515,7.1778))
		{//Fishplace at the bigwheel
			return 1;
		}
	}
...
public SyncTime()
{
	...
	if ((tmphour > ghour) || (tmphour == 0 && ghour == 23))
	{
		...
		foreach(new i: Player)
		{
			if(PlayerInfo[i][pLevel] <= 5) SendClientMessageEx(i,COLOR_LIGHTBLUE,"Viec su dung hack/cleo tai may chu GvR ban se bi xoa tai khoan vinh vien khoi he thong!");
...
stock SendAdvisorMessage(color, string[])
{
	foreach(new i: Player)
	{
		if((PlayerInfo[i][pHelper] >= 1 || PlayerInfo[i][pDonateRank] == 5 || PlayerInfo[i][pWatchdog] == 1) && advisorchat[i])
		{
			SendClientMessageEx(i, color, string);
		}
		else
...
stock str_replace_prefix(sSearch[], sReplace[], const sSubject[], &iCount = 0)
{
	new
...
stock ShowInventory(playerid,targetid)
{
	if(IsPlayerConnected(targetid))
	{
		...
		number_format(PlayerInfo[targetid][pTienban]),
		...
		number_format(PlayerInfo[targetid][pDau]),
...
stock DisplayOrders(playerid)
{
	...
	if (!szDialog[0] || IsABoat(GetPlayerVehicleID(playerid)))
	{
		...
		TogglePlayerControllable(playerid, 1);
		DeletePVar(playerid, "IsFrozen"); */
		if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 414 || GetVehicleModel(GetPlayerVehicleID(playerid)) == 482 || IsABoat(GetPlayerVehicleID(playerid)))
		{
			ShowPlayerDialog(playerid,DIALOG_LOADTRUCKOLD,DIALOG_STYLE_LIST,"Ban muon van chuyen gi?","{00F70C}Hang hoa hop phap {FFFFFF}(hop phap va khong co tien thuong)\n{FF0606}Hang Cam{FFFFFF}(Vu Khi) (vat lieu) (ma tuy)","Lua chon","Huy bo");
		}
		else
		{
		...
		if(playerid == MAX_PLAYERS || PlayerInfo[playerid][pAdmin] >= 2)
		{
			...
			if(randgift >= 1 && randgift <= 83)
			{
				...
				if(gift == 1)
				{
					...
					GivePlayerValidWeapon(giveplayerid, 27, 200);
					GivePlayerValidWeapon(giveplayerid, 24, 200);
					GivePlayerValidWeapon(giveplayerid, 31, 200);
					GivePlayerValidWeapon(giveplayerid, 34, 200);
					GivePlayerValidWeapon(giveplayerid, 29, 200);
