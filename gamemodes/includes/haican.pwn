...
public OnPlayerHarvestMarijuana(playerid, iMar) {
	...
	SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban da nhan duoc 1 can sa !");
	SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban da nhan duoc 1 Goi pot !");
	// Marijuana Var
...
public OnPlayerCBCannabisss(playerid) {
	if(PlayerInfo[playerid][pGoiPot] >= 20) {
		...
		SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban nhan duoc 1 pot");
	}
	else {
		SetPVarInt(playerid, "DangCheBien", 0);
		TogglePlayerControllable(playerid, true); // Freeze player
		SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban da doi thanh cong");
...
public OnPlayerCBCannabis(playerid) {
	if(PlayerInfo[playerid][pCansa] >= 2) {
		...
		SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban da nhan 1 tui can sa!");
	}
	else {
		SetPVarInt(playerid, "DangCheBien", 0);
		TogglePlayerControllable(playerid, true); // Freeze player
		SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban da che bien thanh cong!");
		KillTimer(PlayerInfo[playerid][pChebienTime]);
	}
	return 1;
}
...
public OnPlayerEnterDynamicCP(playerid, checkpointid) {
	if(checkpointid == DynamicCP_Harvest[0]) {// Che bien
		if(PlayerInfo[playerid][pCansa] >= 2) { // 2 can sa = 1 tui can sa
			...
			SendClientMessage(playerid, -1,"[{00b300}Marijuana GvR{ffffff}]: Dang che bien!!!!");
			PlayerInfo[playerid][pChebienTime] = SetTimerEx("OnPlayerCBCannabis", TIME_CHEBIEN_CANSA*1000, 1, "i", playerid);
			}
		}
		else
			SendClientMessage(playerid, -1,"[{00b300}Marijuana GvR{ffffff}]: Ban can phai co 2 can sa de co the che bien!");
	}
	if(checkpointid == DynamicCP_Harvest[1]) { // Ban can sa
		if(PlayerInfo[playerid][pTuicansa] > 0) {
			...
			SendClientMessage(playerid, -1,"[{00b300}Marijuana GvR{ffffff}]: Ban can phai co tui can sa de buoc vao day!");
	}
	if(checkpointid == DynamicCP_Harvest[2]) {
		if(PlayerInfo[playerid][pGoiPot] >= 20) {
			...
			SendClientMessage(playerid, -1,"[{00b300}Marijuana GvR{ffffff}]: Ban can phai co 20 Goi pot");
	}
	return 1;
}
		...
		if(response) {
			if(strval(inputtext) > 0) {
				if(PlayerInfo[playerid][pTuicansa] >= strval(inputtext)) {
					...
					format(string, 128, "[{00b300}Marijuana GvR{ffffff}]: Ban da ban %d tui can va nhan duoc $%d", strval(inputtext), strval(inputtext)*GIATUICANSA);
					SendClientMessage(playerid, -1, string);
				}
				else
					SendClientMessage(playerid, -1,"[{00b300}Marijuana GvR{ffffff}]: So luong khong hop le! hay kiem tra lai so luong cua ban");
			}
			else
				SendClientMessage(playerid, -1,"[{00b300}Marijuana GvR{ffffff}]: So luong can ban phai lon hon 0!");
		}
	}
	return 1;
}
...
CMD:haican(playerid) {
    ...
    if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Vui long xuong xe");
	if(CopOnline <= 0) return  SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Server can co cops moi co the lam viec nay!");
	...
	if(iMarijuana == INVALID_FLOAT) SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban khong dung gan cay nao de thu hoach ca!");
	else {
		if(Marijuana[iMarijuana][marijuana_status] == false) {
			if(HarvestCan[playerid] == false) {
				if(PlayerInfo[playerid][pCansa] < 50) {
					...
					SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban dang hai can sa ...");
					TimeHarvest[playerid] = SetTimerEx("OnPlayerHarvestMarijuana", THUHOACH_TIME*1000, 0, "ii", playerid, iMarijuana); // Timer thu hoach
				}
				else
					SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Can sa da day` ! Ban can di che bien");
			}
			else
				SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Ban khong the thuc hien vao luc nay!");
		}
		else
			SendClientMessage(playerid, -1, "[{00b300}Marijuana GvR{ffffff}]: Cay nay dang co nguoi thu hoach");
	}
	return 1;
}
