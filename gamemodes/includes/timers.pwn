...
task ProductionUpdate[200000]()
{
	...
	foreach(new i: Player)
	{
		...
		if(PlayerInfo[i][pLevel] < 1) {
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Ban Can Dat Cap Do 2 Moi Co The Su dung Vu Khi");
		}
		if(PlayerInfo[i][pTeleJob] > 1) {
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "[Thong Bao]{09FF57}Ban con luot su dung /telejob hay su dung no khi can nhe");
		}
		if(PlayerInfo[i][pGiayPhepVuKhi] < 1) {
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "[Thong Bao]{09FF57}Ban chua co giay phep su dung vu khi hay lam ho so");
		}
	...
	foreach(new i: Player)
	{
	    ...
	    if(GetPVarType(i, "Injured"))
	    {
	        ...
	        if(GetPVarInt(i, "EMSAttempt") != 0)
			{
				...
				{
					if(GetPlayerAnimationIndex(i) != 746)
		...
		if(playerTabbed[i] == 0) {
			if(PlayerInfo[i][pJailTime] > 0 && --PlayerInfo[i][pJailTime] <= 0) {
				if(strfind(PlayerInfo[i][pPrisonReason], "[IC]", true) != -1 || strfind(PlayerInfo[i][pPrisonReason], "[ISOLATE]", true) != -1) {
					...
					PlayerInfo[i][pVW] = 0;
					SetPlayerPos(i, 28.4767,306.0742,6.3080);
				}
				else {
				 	SetPlayerInterior(i, 0);
					PlayerInfo[i][pInt] = 0;
