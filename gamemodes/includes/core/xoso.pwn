...
CMD:muaveso(playerid, params[])
{
	...
	{
		strdel(string, 0, 1);
		ShowPlayerDialog(playerid, DIALOG_MUAVESO, DIALOG_STYLE_INPUT, "VNC | XO SO", string, "Mua ngay", "Huy bo");
	}
	return 1;
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
		...
		if(PlayerInfo[playerid][pVeSo] == 0)
		{
		    if(veso >= 0 && veso < 100)
		    {
		        if(veso < 0)
		        {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_TIENCUOC, DIALOG_STYLE_INPUT, "VNC | XO SO", "Ban vui long nhap so tien cuoc cho con so may man ban vua mua: ", "Dat cuoc", "Huy bo");
				}
	...
	{
		if(strfind(inputtext, "%", true) != -1) {
		    return ShowPlayerDialog(playerid, DIALOG_TIENCUOC, DIALOG_STYLE_INPUT, "VNC | XO SO", "Ban vui long nhap so tien cuoc cho con so may man ban vua mua (0 ~ 9999KC): ", "Dat cuoc", "Huy bo");
		}
		if(strfind(inputtext, "c", true) != -1) {
			return ShowPlayerDialog(playerid, DIALOG_TIENCUOC, DIALOG_STYLE_INPUT, "VNC | XO SO", "Ban vui long nhap so tien cuoc cho con so may man ban vua mua (0 ~ 9999KC): ", "Dat cuoc", "Huy bo");
		}
		    ...
		    if(PlayerInfo[playerid][pnDP] >= tiencuoc)
		    {
			    if(tiencuoc > 0 && tiencuoc < 9999)
			    {
			        if(tiencuoc < 0)
			        {
			            ...
			            ShowPlayerDialog(playerid, DIALOG_TIENCUOC, DIALOG_STYLE_INPUT, "VNC | XO SO", "Ban vui long nhap so tien cuoc cho con so may man ban vua mua (0 ~ 9999KC): ", "Dat cuoc", "Huy bo");
						return 1;
...
CMD:testquayso(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] >= 7)
	{
		...
		SendClientMessageToAllEx(COLOR_YELLOW, "[VNG-RP] KET QUA XO SO , DA DUOC BAT DAU {00ff00}(/xemxoso)." );
