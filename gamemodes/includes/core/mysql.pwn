...
public OnQueryFinish(resultid, extraid, handleid)
{
	...
	switch(resultid)
	{
		...
		case IPBAN_THREAD:
		{
		    if(rows > 0)
			{
				SendClientMessage(extraid, COLOR_RED, "IP cua ban bi cam truy cap, de mo khoa vui long truy cap dien dan www.rog-rp.com");
				SetTimerEx("KickEx", 1000, 0, "i", extraid);
			}
			else
			{
...
public OnLoadHGBackpacks()
{
	...
	while((index < rows))
	{
		...
		format(string, sizeof(string), "Tui do Hunger Game\nType: %s\n{FF0000}(ID: %d){FFFFFF}", GetBackpackName(index), index);
