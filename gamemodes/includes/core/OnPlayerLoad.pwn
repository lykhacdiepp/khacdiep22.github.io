...
public OnPlayerLoad(playerid)
{
	...
	if(PlayerInfo[playerid][pReg] == 0)
	{
		...
		PlayerInfo[playerid][pFe] = 0;
		PlayerInfo[playerid][pCu] = 0;
	...
	if(PlayerInfo[playerid][pAdmin] != 0 && PlayerInfo[playerid][pAdmin] != 1 && PlayerInfo[playerid][pAdmin] != 2 && PlayerInfo[playerid][pAdmin] != 3 && PlayerInfo[playerid][pAdmin] != 4 &&PlayerInfo[playerid][pAdmin] != 5 && PlayerInfo[playerid][pAdmin] != 6 && PlayerInfo[playerid][pAdmin] != 7)
	{
...
/*   TextDrawShowForPlayer(playerid,Gtaaepro0);
   TextDrawShowForPlayer(playerid,Gtaaepro1);
   TextDrawShowForPlayer(playerid,Gtaaepro2);
   TextDrawShowForPlayer(playerid,Gtaaepro3);*/
	...
	SendClientMessageEx( playerid, COLOR_GREEN, "*LUU Y: Khi war/shotfire nhau thi moi thanh vien nen co video bang chung (BDC) de~khang kien phong cho truong hop sau nay ");
	SendClientMessageEx( playerid, COLOR_GREEN, "Dich covid dang lan chuyen khap San Andreas,de nghi tat ca nguoi dan trong thanh pho phai den benh vien tiem vacxin");
	SendClientMessageEx( playerid, COLOR_WHITE, "Goi Y: De kiem viec lam ban hay /map de tim vi tri cong viec");
	SendClientMessageEx( playerid, COLOR_WHITE, " Mua he nang dong,server se co nhung su kien hay trong mua he nay");
	SendClientMessageEx( playerid, COLOR_YELLOW, " Ban da tham gia may chu bang phien ban: SA-MP 0.3.7");
	...
	// Hien IP khi dang nhap
	new name[MAX_PLAYER_NAME];
	GetPlayerName(playerid, name, sizeof(name));
	GetPlayerIp(playerid, ip, sizeof(ip));
	format(string, sizeof(string), "SERVER: %s dang dang nhap, IP Address: %s", name, ip);
	foreach(new i: Player)
	{
