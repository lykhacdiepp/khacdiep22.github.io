...
NationSel_HandleNationSelection(playerid)
{
	...
	if(Keys & KEY_FIRE)
	{
		...
		ShowPlayerDialog(playerid, CHAOMUNGDENVOISERVER, DIALOG_STYLE_MSGBOX, "Chao mung ban den voi Server", "Duoi day la mot so nhung lenh co ban khi moi bat dau tham gia server CLBSAMP:\n+ /newb neu ban muon hoi mot dieu gi day trong game\n+ /yeucautrogiup de nho doi ngu Helper ho tro ban ve cach choi\n+ /baocao neu ban gap loi, su co hoac la gap hacker/cheater", "OK", "");
		SendClientMessage(playerid, COLOR_YELLOW, "Tham gia group: facebook.com/groups/CLBSAMP");
		SendClientMessage(playerid, COLOR_YELLOW, "Ban nhan duoc 1 VIP Silver, /myvouchers hoac /phieucuatoi de kiem tra");
		SendClientMessage(playerid, COLOR_YELLOW, "Bay gio hay /timvieclam va tim cho minh mot cong viec nao. Cong viec de nghi: Khai thac, Lam tac, Pizza boy.");
		SetCameraBehindPlayer(playerid);
		SetPlayerPos(playerid, 1715.1124,-1900.4352,13.5665);
...
ShowCasinoGamesMenu(playerid, dialogid)
{
	switch(dialogid)
	{
		...
		{
			return ShowPlayerDialog(playerid, DIALOG_CGAMESCREDITS, DIALOG_STYLE_MSGBOX, "{FFFFFF}Casino Games - (Creadits)", "{FFFFFF}Chinh sua boi: rog-rp.com", "Quay Lai", "");
		}
	...
	getdate(year, month, day);
 	...
 	if(PlayerInfo[i][pLevel] > 0) {
		...
		if(PlayerInfo[i][pConnectSeconds] >= 3600) {
			...
			{
				format(string, sizeof(string), "  Paycheck: $%s  |  SA Gov Tax: $%s (%d phan tram)", number_format(PlayerInfo[i][pPayCheck] / 6), number_format((PlayerInfo[i][pPayCheck] / 100) * TaxValue), TaxValue);
			...
			{
				format(string, sizeof(string), "  Paycheck: $%s  |  TR Gov Tax: $%s (%d phan tram)", number_format(PlayerInfo[i][pPayCheck] / 9), number_format((PlayerInfo[i][pPayCheck] / 100) * TRTaxValue), TRTaxValue);
			...
			switch(PlayerInfo[i][pDonateRank]) {
				case 0: {
					...
					format(string, sizeof(string), "  Tien ngan hang: $%s  |  Lai suat: 0.1 phan tram (1000 max)", number_format(PlayerInfo[i][pAccount]));
					SendClientMessageEx(i, COLOR_GRAD1, string);
				}
				case 1: {
					if(interest > 2000) interest = 2000;
					format(string, sizeof(string), "  Tien ngan hang: $%s  |  Lai suat: 0.1 phan tram {FFFF00}(Bronze VIP: 2000 max)", number_format(PlayerInfo[i][pAccount]));
					SendClientMessageEx(i, COLOR_GRAD1, string);
				}
				case 2:	{
					if(interest > 3000) interest = 3000;
					format(string, sizeof(string), "  Tien ngan hang: $%s  |  Lai suat: 0.1 phan tram {FFFF00}(Silver VIP: 3000 max)", number_format(PlayerInfo[i][pAccount]));
					SendClientMessageEx(i, COLOR_GRAD1, string);
				}
				case 3: {
					if(interest > 4000) interest = 4000;
					format(string, sizeof(string), "  Tien ngan hang: $%s  |  Lai suat: 0.1 phan tram {FFFF00}(Gold VIP: 4000 max)", number_format(PlayerInfo[i][pAccount]));
					SendClientMessageEx(i, COLOR_GRAD1, string);
				}
				case 4, 5: {
					if(interest > 5000) interest = 5000;
					format(string, sizeof(string), "  Tien ngan hang: $%s  |  Lai suat: 0.1 phan tram {FFFF00}(Platinum VIP: 5000 max)", number_format(PlayerInfo[i][pAccount]));
					SendClientMessageEx(i, COLOR_GRAD1, string);
				}
			}
			...
			format(string, sizeof(string), "  Tien ngan hang (moi'): $%s  |  Tra thue: $%s", number_format(PlayerInfo[i][pAccount]), number_format((0 <= PlayerInfo[i][pRenting] < sizeof HouseInfo) ? (HouseInfo[PlayerInfo[i][pRenting]][hRentFee]) : (0)));
...
//Rua tien
forward ThoiGianRuaTien(playerid);
public ThoiGianRuaTien(playerid)
{
	ClearAnimations(playerid);
	TogglePlayerControllable(playerid, 1);
	DeletePVar(playerid, "ThoiGianRuaTien");
	RuaTien[playerid] = 0;
	new string[256], soluongtienban = PlayerInfo[playerid][pDirtyMoney];
	format(string, sizeof(string), "Ban da tien hanh rua tien va nhan duoc so tien la %d", soluongtienban);
	SendClientMessageEx(playerid, COLOR_GRAD1, string);
	PlayerInfo[playerid][pDirtyMoney] -= soluongtienban;
	GivePlayerCash(playerid, soluongtienban);
}


//San xuat can sa
forward ThoiGianSanXuatCanSa(playerid);
public ThoiGianSanXuatCanSa(playerid)
{
	ClearAnimations(playerid);
	TogglePlayerControllable(playerid, 1);
	DeletePVar(playerid, "ThoiGianSanXuatCanSa");
	SanXuat[playerid] = 0;
	PlayerInfo[playerid][pCayCanSa] -= 2;
	PlayerInfo[playerid][pPot] += 1;
	new string[256];
	format(string, sizeof(string), "Ban da dung 2 cay ma tuy de san xuat va nhan duoc mot goi can sa nguyen chat [%d/20]", PlayerInfo[playerid][pPot]);
	SendClientMessageEx(playerid, COLOR_GRAD1, string);	
}

//Thu hoach can sa
forward ThoiGianThuHoachCanSa(playerid);
public ThoiGianThuHoachCanSa(playerid)
{
	ClearAnimations(playerid);
	TogglePlayerControllable(playerid, 1);
	CP[playerid] = 6;
	DeletePVar(playerid, "ThoiGianThuHoachCanSa");
	ThuHoach[playerid] = 0;
	PlayerInfo[playerid][pCayCanSa] += 1;
	new string[256];
	format(string, sizeof(string), "Ban nhan duoc mot cay can sa [%d/20]", PlayerInfo[playerid][pCayCanSa]);
	SendClientMessageEx(playerid, COLOR_GRAD1, string);
}

//San xuat ma tuy
forward ThoiGianSanXuatMaTuy(playerid);
public ThoiGianSanXuatMaTuy(playerid)
{
	ClearAnimations(playerid);
	TogglePlayerControllable(playerid, 1);
	DeletePVar(playerid, "ThoiGianSanXuatMaTuy");
	SanXuat[playerid] = 0;
	PlayerInfo[playerid][pCayMaTuy] -= 2;
	PlayerInfo[playerid][pCrack] += 1;	
	new string[256];
	format(string, sizeof(string), "Ban da dung 2 cay ma tuy de san xuat va nhan duoc mot goi ma tuy nguyen chat [%d/20]", PlayerInfo[playerid][pCrack]);
	SendClientMessageEx(playerid, COLOR_GRAD1, string);	
}

//Thu hoach ma tuy
forward ThoiGianThuHoachMaTuy(playerid);
public ThoiGianThuHoachMaTuy(playerid)
{
	ClearAnimations(playerid);
	TogglePlayerControllable(playerid, 1);
	CP[playerid] = 5;
	DeletePVar(playerid, "ThoiGianThuHoachMaTuy");
	ThuHoach[playerid] = 0;
	PlayerInfo[playerid][pCayMaTuy] += 1;
	new string[256];
	format(string, sizeof(string), "Ban nhan duoc mot cay ma tuy [%d/20]", PlayerInfo[playerid][pCayMaTuy]);
	SendClientMessageEx(playerid, COLOR_GRAD1, string);
}
...
// Job lam tac
forward ThoiGianCuaCay(playerid);
public ThoiGianCuaCay(playerid)
{
	ClearAnimations(playerid);
	SetPlayerSpecialAction(playerid, SPECIAL_ACTION_CARRY);
	TogglePlayerControllable(playerid, 1);
	RemovePlayerAttachedObject(playerid, 1);
	SetPlayerCheckpoint(playerid, -12.62,975.02,19.80,3);
	CP[playerid] = 4;
	SetPlayerAttachedObject(playerid, 1, 1463, 1,0.20,0.36,0.0,0.0,90.0, 0.0, 0.4, 0.3, 0.6); 
	DeletePVar(playerid, "ThoiGianCuaCay");
	CuaCay[playerid] = 0;
	SendClientMessage(playerid, -1, "Hay giao tra go cho xuong go de hoan thanh cong viec va nhan tien");
}


// Job khai thac 
forward ThoiGianKhaiThac(playerid);
public ThoiGianKhaiThac(playerid)
{
	ClearAnimations(playerid);
	SetPlayerSpecialAction(playerid, SPECIAL_ACTION_CARRY);
	TogglePlayerControllable(playerid, 1);
	RemovePlayerAttachedObject(playerid, 1);
	SetPlayerCheckpoint(playerid, -730.75,1517.90,38.63,3);
	CP[playerid] = 3;
	...
	DeletePVar(playerid, "ThoiGianKhaiThac");
	KhaiThac[playerid] = 0;
	SendClientMessage(playerid, -1, "Hay giao tra da' cho nha may de hoan thanh cong viec va nhan tien");
}


//Ve sinh moi truong
forward ThoiGianVeSinh(playerid);
public ThoiGianVeSinh(playerid)
{
	TogglePlayerControllable(playerid, 1);
	DeletePVar(playerid, "ThoiGianVeSinh");
	DonVeSinh[playerid] = 0;
	SetPlayerCheckpoint(playerid, 816.2760,-602.8568,16.8967, 5);
	SendClientMessageEx(playerid, -1, "Ban da don ve sinh moi truong xong hay quay tro ve tra xe va nhan tien!");	
	CP[playerid] = 7;
}

//Tha Luoi
forward ThoiGianThaLuoi(playerid);
public ThoiGianThaLuoi(playerid)
{
	TogglePlayerControllable(playerid, 1);
	DeletePVar(playerid, "ThoiGianThaLuoi");
	ThaLuoi[playerid] = 0;
	SendClientMessage(playerid, -1, "Ban nhan duoc 150$ cho viec danh bat cua minh, hay quay ve cho tha luoi neu muon lam tiep!");
...
public LoadGame()
{
    ...
    new msg[128];
    format(msg,sizeof(msg),"Bai toan cho 200$ neu ai tra loi dung --> %i + %i - %i + %i = ?",E_NUM1,E_NUM2,E_NUM3,E_NUM4);
...
stock SetPlayerJoinCamera(playerid)
{
	...
	switch(randcamera)
	{
		...
		case 7: // All Saints
		{
			SetPlayerVirtualWorld(playerid, 0);
			SetPlayerInterior(playerid, 0);
			Streamer_UpdateEx(playerid,1259.1559,355.3452,37.8411);
			SetPlayerPos(playerid, 1259.1559,355.3452,37.8411);
			SetPlayerCameraPos(playerid,1259.1559,355.3452,37.8411+6.0);
			SetPlayerCameraLookAt(playerid,1259.1559,355.3452,37.8411);
		}
		case 8: // Unity
		{
			SetPlayerVirtualWorld(playerid, 0);
...
stock GetJobName(job)
{
	...
	switch(job)
	{
		...
		case 22: name = "Khai thac";
		case 23: name = "Lam tac";
	...
	if(GetPVarInt(playerid, "MedicBill") == 1 && PlayerInfo[playerid][pJailTime] == 0)
	{
		switch(PlayerInfo[playerid][pHospital]) {
			case 1:
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien: $150!"); // All Saint
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Chi nhanh benh vien: All Saints. Chuc mot ngay tot lanh!"); // All Saint
				...
				for(new z; z < MAX_GROUPS; z++)
				{
					if(arrGroupData[z][g_iAllegiance] == 1)
					{
						if(arrGroupData[z][g_iGroupType] == 5)
						{
							...
							getdate(year,month,day);
							Tax += 150;
							format(str, sizeof(str), "%s has their medical fees, adding $150 to the vault.", GetPlayerNameEx(playerid));
							format(file, sizeof(file), "grouppay/%d/%d-%d-%d.log", z, month, day, year);
							Log(file, str);
							break;
						}
			...
			case 8:
			{
				...
				Misc_Save();
				SendClientMessageEx(playerid, TEAM_CYAN_COLOR, "Hoa don benh vien $250. Chuc mot ngay tot lanh!!");
				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da hoi sinh tu nha cua ban.");
				...
				for(new i = 0; i < sizeof(HouseInfo); i++)
				{
					if(PlayerInfo[playerid][pPhousekey] == i || PlayerInfo[playerid][pPhousekey2] == i)
					{
						...
						GameTextForPlayer(playerid, "~w~Chao mung da tro ve", 5000, 1);
    ...
    if(IsPlayerConnected(playerid))
	{
		...
		if(HungerPlayerInfo[playerid][hgInEvent] == 1)
		{
			if(hgActive > 0)
			{
				...
				{
					new szmessage[128];
					format(szmessage, sizeof(szmessage), "** %s da dat duoc top 3 trong event Hunger Games.", GetPlayerNameEx(playerid));
					...
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "** Ban nhan duoc $100 cho su kien Hunger Games");
				...
				{
					new szmessage[128];
					format(szmessage, sizeof(szmessage), "** %s da dat duoc top 2 trong event Hunger Games.", GetPlayerNameEx(playerid));
					SendClientMessageToAll(COLOR_LIGHTBLUE, szmessage);
					...
					SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "** Ban nhan duoc $200 cho su kien Hunger Games.");
					...
					for(new i = 0; i < MAX_PLAYERS; i++)
					{
						...
						{
							format(szmessage, sizeof(szmessage), "** %s da dat duoc top 1 trong event Hunger Games.", GetPlayerNameEx(i));
							SendClientMessageToAll(COLOR_LIGHTBLUE, szmessage);
							...
							GivePlayerCash(playerid, 500);
							SendClientMessageEx(i, COLOR_LIGHTBLUE, "** Ban nhan duoc $500 cho su kien Hunger Games.");
							hgActive = 0;
		...
		if(PlayerInfo[playerid][pTut] == 0)
		{
			...
			//Streamer_UpdateEx(playerid, 2229.4968,-1722.0701,13.5625);
			SetPlayerPos(playerid, -67.8489,176.2209,2.7664);
			SetPlayerCameraPos(playerid, 50.0000,50.0000,50.0000);
			SetPlayerCameraLookAt(playerid, 70.000,70.000,70.000);
...
stock IsAXeVeSinh(carid)
{
	for (new v = 0; v < sizeof(XeRacVehicles); v++) {
	    if(carid == XeRacVehicles[v]) return 1;
	}
	return 0;
}

stock IsAThuyenNguDan(carid)
{
	...
	switch (item)
  	{
		...
		case ITEM_ALOCK: // sua bang lenh hay sao ay quen roi
		{
...
stock Misc_Load() {
	...
	if(iRewardBox) {
		...
		tRewardText = CreateDynamic3DTextLabel("Hop Qua Vang\n{FFFFFF}/nhanquavang{F3FF02} de nhan qua cua ban!", COLOR_YELLOW, fObjectPos[0], fObjectPos[1], fObjectPos[2], 10.0, .testlos = 1, .streamdistance = 50.0);
	}
	fclose(iFileHandle);
				...
				if (vehicleSpeed > speedLimit && PlayerInfo[p][pTicketTime] == 0)
				{
					...
					if(!IsAPlane(vehicleid) && !IsAHelicopter(vehicleid) && GetVehicleModel(vehicleid) != 481 && GetVehicleModel(vehicleid) != 509 && GetVehicleModel(vehicleid) != 510)
					{
					    foreach(new i: Player)
						{
	        	    	    ...
	        	    	    if(v != -1)
							{
								new string[128], Amount = floatround(10*(vehicleSpeed-speedLimit), floatround_round);
...
stock DisplayOrders(playerid)
{
	...
	// xe truck // truck
		...
		TogglePlayerControllable(playerid, 1);
		DeletePVar(playerid, "IsFrozen"); */
		if(GetVehicleModel(GetPlayerVehicleID(playerid)) == 414 || GetVehicleModel(GetPlayerVehicleID(playerid)) == 414 || IsABoat(GetPlayerVehicleID(playerid)))
		{
		...
		case 0: // Pocket or On Hand
		{
			if(itemid == 1)
			{
				...
				if(fromplayerid != -1 && fromstorageid != -1 && playerid != fromplayerid) {
					...
					{
						format(string, sizeof(string), "[Warning] %s (IP:%s) da dua $%s %s cho %s (IP:%s)", GetPlayerNameEx(fromplayerid), GetPlayerIpEx(fromplayerid), number_format(amount), itemtype[itemid], GetPlayerNameEx(playerid), GetPlayerIpEx(playerid));
						Log("logs/adminpay.log", string);
						format(string, sizeof(string), "{AA3333}AdmWarning{FFFF00}: %s (IP:%s) da dua $%s %s cho %s (IP:%s)", GetPlayerNameEx(fromplayerid), GetPlayerIpEx(fromplayerid), number_format(amount), itemtype[itemid], GetPlayerNameEx(playerid), GetPlayerIpEx(playerid));
						ABroadCast(COLOR_YELLOW, string, 2);
					}
					else if(PlayerInfo[fromplayerid][pAdmin] < 2)
					{
						...
						if(amount >= 100 && PlayerInfo[fromplayerid][pLevel] <= 3 && itemid == 1) ABroadCast(COLOR_YELLOW, string, 2);
						if(amount >= 1000 && itemid == 1)	ABroadCast(COLOR_YELLOW,string,2);
					}
				}
				return 1;
			}
...
stock GetStaffRank(playerid)
{
	...
	else if(PlayerInfo[playerid][pAdmin] > 1)
	{
		switch(PlayerInfo[playerid][pAdmin])
		{
			...
			case 5: format(string, sizeof(string), "{FF0000}Head Administrator{FFFFFF}");
			case 6: format(string, sizeof(string), "{298EFF}Lead Head Administrator{FFFFFF}");
			case 7: format(string, sizeof(string), "{298EFF}Executive Administrator{FFFFFF}");
...
public SuDungGiftCode(playerid,type)
{
	if(type == 1)
	{
		...
		SendClientMessageEx(playerid, -1, "Ban nhan duoc 1 phieu xe - /phieucuatoi de kiem tra!");
	    return 1;
	}
	else if(type == 2)
	{
	    GivePlayerCash(playerid,20000);
		SendClientMessageEx(playerid, -1, "Ban nhan duoc $20,000!");
	    return 1;
	}
	return 1;
}
