...
timer NOPCheck[5000](playerid)
{
	...
	else if(KiemTraXeCanhSat(newcar) && (arrGroupData[PlayerInfo[playerid][pMember]][g_iGroupType] == 0 || arrGroupData[PlayerInfo[playerid][pLeader]][g_iGroupType] == 0)) ExecuteNOPAction(playerid);
	else if(KiemTraXeCanhSat(newcar) && (arrGroupData[PlayerInfo[playerid][pMember]][g_iGroupType] == 2 || arrGroupData[PlayerInfo[playerid][pLeader]][g_iGroupType] == 2)) ExecuteNOPAction(playerid);
...
task thoigiann[1000]() 
{
    ...
    getdate(year, month, day);
    format(string, sizeof(string), "%02d:%02d", hour, minuite);
    TextDrawSetString(Gtaaepro2, string);
    format(string, sizeof(string), "%d/%d/%d", day, month,year);
    TextDrawSetString(Gtaaepro0, string);
	return true;
}
...
task ProductionUpdate[300000]()
{
	...
	foreach(new i: Player)
	{
		...
		{
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Can ho tro? Doi ngu Helper co the giup do ban. (/yeucautrogiup de duoc giup do)");
		}
		if(PlayerInfo[i][pConnectHours] < 2) {
			SendClientMessageEx(i, COLOR_LIGHTRED, "Vu khi se bi gioi han 2 gio choi dau cho nguoi moi tham gia cong dong GTA-Viet.");
		}
...
task ServerHeartbeat[1000]() {
	...
	foreach(new i: Player)
	{
		...
		if (PlayerInfo[i][pAdmin] < 2 || HelpingNewbie[i] != INVALID_PLAYER_ID)
		{
			...
			{
				if (++PlayerInfo[i][pHungerTimer] >= 10000 && PlayerInfo[i][pHunger] > 0) // 72 giay la bi -2 trang thai, tuc la 100 hunger thi 1 tieng moi doi bung
				{
				...
				if(PlayerCuffed[i] == 0)
				{
					if (PlayerInfo[i][pHunger] == 0 && ++PlayerInfo[i][pHungerDeathTimer] >= 300) // 5 phut de di an
					{
...
{	
	...
	if(hgActive)
	{
		...
		{	
			...
			if(hgCountdown == 300)
			{
				SendClientMessageToAll(COLOR_LIGHTBLUE, "Su kien Hunger Games se bat dau trong 5 phut nua, su dung /thamgiahg de tham gia.");
			}
			else if(hgCountdown == 60) 
			{
				SendClientMessageToAll(COLOR_LIGHTBLUE, "Su kien Hunger Games se bat dau trong 1 phut nua, type /thamgiahg de tham gia.");
			}
			else if(hgCountdown == 30)
			{
				for(new i = 0; i < MAX_PLAYERS; i++)
				{
					if(HungerPlayerInfo[i][hgInEvent] == 1)
					{
						...
						SendClientMessageEx(i, COLOR_LIGHTBLUE, "* Godmode bi vo hieu hoa, balo se suat hien sau 30 giay...");
					}
				}
			}
		}
		else if(hgCountdown == 0 && hgActive == 1)
		{
			...
			for(new i = 0; i < MAX_PLAYERS; i++)
			{
				if(HungerPlayerInfo[i][hgInEvent] == 1)
				{
					...
					SendClientMessageEx(i, COLOR_LIGHTBLUE, "* Hunger Games da bat dau!");
					GameTextForPlayer(i, "GAME START!", 2000, 6);
