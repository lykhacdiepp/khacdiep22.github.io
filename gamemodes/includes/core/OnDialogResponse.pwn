public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == ACTOR_DRUGSDEALER)
    {
        if(response)
        {
			...
			if(listitem == 2) //doi sung
            {
              format(string,sizeof(string),"9mm(10 Fe)\nshot gun(25 Fe)\nMp5(20Cu)\nM4(50cu)\ncay gay", "Co", "Khong");
    ...
    if(dialogid == BANCANSA)
    {
        ...
        format(string, sizeof(string), "Ban da ban' %d can sa nguyen chat va nhan duoc %d tien ban [ /qs ]", soluongcansa, soluongcansa*200);
        SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
        PlayerInfo[playerid][pDirtyMoney] += soluongcansa*5000;
    }
    ...
    if(dialogid == BANMATUY)
    {
        ...
        SendClientMessageEx(playerid, COLOR_LIGHTBLUE, string);
        PlayerInfo[playerid][pDirtyMoney] += soluongmatuy*5000;
    }

	if(dialogid == MUAVUKHI)
    {
        if(response)
        {
            if(listitem == 0)
            {
				...
				PlayerInfo[playerid][pFe] -= 10;
				SendClientMessageEx(playerid, -1, "Ban da dung tien ban de mua mot mon vu khi!");
            }
            else if(listitem == 1)
            {
				GivePlayerValidWeapon(playerid, 25, 60000);
				PlayerInfo[playerid][pFe] -= 25;
				SendClientMessageEx(playerid, -1, "Ban da dung tien ban de mua mot mon vu khi!");
            }
			else if(listitem == 2)
            {
				GivePlayerValidWeapon(playerid, 29, 60000);
				PlayerInfo[playerid][pCu] -= 20;
				SendClientMessageEx(playerid, -1, "Ban da dung tien ban de mua mot mon vu khi!");
            }
			else if(listitem == 3)
            {
				GivePlayerValidWeapon(playerid, 31, 60000);
				PlayerInfo[playerid][pCu] -= 50;
				SendClientMessageEx(playerid, -1, "Ban da dung tien ban de mua mot mon vu khi!");
            }
			else if(listitem == 4)
            {
				GivePlayerValidWeapon(playerid, 5, 60000);
				PlayerInfo[playerid][pDirtyMoney] -= 1000;
				SendClientMessageEx(playerid, -1, "Ban da dung tien ban de mua mot mon vu khi!");
            }
        }
    }
	...
	if(dialogid == ACTOR_CRACK)
    {
        if(response)
        {
            ...
            if(listitem == 2) //doi sung
		    {
				ShowPlayerDialog(playerid, MUAVUKHI, DIALOG_STYLE_MSGBOX, "vu khi", "9mm(10 Fe)/nshot gun(25 Fe)/nMp5(20Cu)/nM4(50cu)/ncay gay", "Co", "Khong");
	...
	// Actor Xin viec 
	if(dialogid == ACTOR_XINVIEC) // xin viec
    {
        if(response)
        {
			...
			if(listitem == 1) //Nhan dong phuc
		    {
				if(PlayerInfo[playerid][pJob] == 1) {
                	SendClientMessageEx(playerid, COLOR_WHITE, "Ban da nhan duoc dong phuc!");
					//PlayerInfo[playerid][pModel] = 153;
	...
	if(dialogid == CHAPNHANCONGVIEC)
	{
		if(response)
		{
			return cmd_chapnhan(playerid, "congviec");
		}
		else
		{
			ShowPlayerDialog(playerid, KHONGCHAPNHAN_CONGVIEC, DIALOG_STYLE_MSGBOX, "Cong viec", "Ban da khong nhan cong viec nay", "Ok", "");
		}
		return 1;
	}		
	...
	// Tro giup newbie moi tham gia
	if(dialogid == CHAOMUNGDENVOISERVER)
	{
		if(response)
		{
			ShowPlayerDialog(playerid, YEUCAUTROGIUPNEWBIE, DIALOG_STYLE_MSGBOX, "Chao mung ban den voi Server GTA-Viet", "Ban co muon nhan su tro giup cua doi ngu Helper ngay bay gio khong ?", "Co", "Khong");
		}
		else
		{
			ShowPlayerDialog(playerid, YEUCAUTROGIUPNEWBIE, DIALOG_STYLE_MSGBOX, "Chao mung ban den voi Server GTA-Viet", "Ban co muon nhan su tro giup cua doi ngu Helper ngay bay gio khong ?", "Co", "Khong");
	...
	if(dialogid == YEUCAUTROGIUPNEWBIE)
	{
		if(response)
		{
			return cmd_yeucautrogiup(playerid, "");
		}
		else
		{
			ShowPlayerDialog(playerid, WELCOMECENT, DIALOG_STYLE_MSGBOX, "Chao mung ban den voi Server GTA-Viet", "Ok chuc ban may man, nho /yeucautrogiup hoac /newb neu ban co thac mac", "OK", "");
		}
		return 1;
	}

    if(dialogid == DIALOG_GIFTCODE)
	{
	    if(response)
	    {
			//new giftcode = strval(inputtext);
			file_Open("GiftCode.ini");
			if(file_IsKey(inputtext))
			{
			CallLocalFunction("SuDungGiftCode","ii",playerid,file_GetVal(inputtext));
			file_RemoveKey(inputtext);
			}
			else
			{
	...
	if(dialogid == AUDIO_URL_LIST)
	{
	    ...
	    {
		 	// 181 Power
		 	SendAudioURLToRange("https://i.top4top.io/m_2721fxdi00.mp3",GetPVarFloat(playerid, "pBoomBoxX"),GetPVarFloat(playerid, "pBoomBoxY"),GetPVarFloat(playerid, "pBoomBoxZ"),50);
	...
	if(dialogid == TIMVIECLAM)
    {
        if(response)
        {
            ...
            else if(listitem == 1)// Pizza Boy
            {
                SetPlayerCheckpoint(playerid, -1719.27,1355.83,7.64, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
            }
            else if(listitem == 2)// Khai thac da
            {
                SetPlayerCheckpoint(playerid, -728.33,1532.11,40.06, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
            }
            else if(listitem == 3)// Tieu phu
            {
                SetPlayerCheckpoint(playerid, 0.29,968.90,18.74, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
            }
            else if(listitem == 4)// ngu dan
            {
                SetPlayerCheckpoint(playerid, 151.10,-1826.43,3.76, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
	        }
            else if(listitem == 5)// don rac
            {
                SetPlayerCheckpoint(playerid, 810.6426,-613.7755,16.3359, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
	        }
            else if(listitem == 6)// tro giup can sa
            {
            return cmd_hidetrogiupcansa(playerid, "");
            }
        }
    }
    // tong hop
   	if(dialogid == TONGHOP)
    {
        if(response)
        {
            ...
            {
                return cmd_hidetimvieclam(playerid, "");
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
            }
            else if(listitem == 1)// Vi tri san xuat
            {
                return cmd_hidetimduong(playerid, "");
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
            }
        }
    }
   	...
   	if(dialogid == TROGIUPCANSA)
    {
        if(response)
        {
            if(listitem == 0)// Vi Tri Hai Can
            {
                SetPlayerCheckpoint(playerid, -2778.73,-29.86,7.18, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
            }
            else if(listitem == 1)// Vi tri san xuat
            {
                SetPlayerCheckpoint(playerid, -2454.59,-136.03,26.19, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
            }
            else if(listitem == 2)// Vi Tri Ban
            {
                SetPlayerCheckpoint(playerid, -2174.87,-265.53,36.51, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
            }
            else if(listitem == 3)// Vi tri rua tien
            {
                SetPlayerCheckpoint(playerid, -1618.99,867.93,7.69, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 1;
	        }
        }
 	...
 	if(dialogid == TIMDUONG)
    {
        if(response)
        {
            if(listitem == 0)// shop xe
            {
                SetPlayerCheckpoint(playerid, -2411.59, -47.52, 35.32, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 1)// bank
            {
                SetPlayerCheckpoint(playerid, -1581.41,902.36,7.69, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 2)// vip
            {
                SetPlayerCheckpoint(playerid, -2439.89, 507.23, 29.94, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 3)// pizza stack
            {
                SetPlayerCheckpoint(playerid, -1719.18,1354.99,7.18, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 4)// benh vien
            {
                SetPlayerCheckpoint(playerid, 2785.553, 2394.641, 1240.531, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 5)// Cua hang 24/7
            {
                SetPlayerCheckpoint(playerid, -2443.35, 755.30, 35.17, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 6)// city hall
            {
                SetPlayerCheckpoint(playerid, -2443.35, 755.30, 35.17, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 7)// cay xang
            {
                SetPlayerCheckpoint(playerid, -2766.44, 375.42, 6.33, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 8)// dmv
            ...
            if(listitem == 9)// shop qa
            {
                SetPlayerCheckpoint(playerid, 626.9684,-571.9612,17.9207, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 10)// shop fix
            {
                SetPlayerCheckpoint(playerid, -1904.20, 275.29, 41.04, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }
            if(listitem == 11)// sua xe
            {
                SetPlayerCheckpoint(playerid, 720.0053,-470.3224,16.3437, 5);
				PlayerPlaySound(playerid,1149,0.0,0.0,0.0);
                CP[playerid] = 2;
            }			
        }
	...
	if(RegistrationStep[playerid] != 0 || strcmp(PlayerInfo[playerid][pBirthDate], "0000-00-00", true) == 0)
	{
		...
		else if(dialogid == REGISTERREF)
		{
		    if(response)
		    {
				...
				SendClientMessage(playerid, COLOR_YELLOW, "{7cfc00}chao mung ban den server");
				SendClientMessage(playerid, COLOR_YELLOW, "{ba55d3}VG Community ");
				SendClientMessage(playerid, COLOR_YELLOW, "neu Ban khong biet choi co the /newb hoac /yeucautrogiup");
				SendClientMessage(playerid, COLOR_YELLOW, "ban co the /map đe kiem viec lam");
				SetCameraBehindPlayer(playerid);
				SetPlayerPos(playerid, -1648.97,1301.58,7.03);
	...
	switch(dialogid)
	{
		...
		case DIALOG_LISTGROUPS: if(response) {
			if (PlayerInfo[playerid][pAdmin] < 5 && !PlayerInfo[playerid][pFactionModerator]) return 1;
		...
		case DIALOG_GROUP_JURISDICTION_LIST: {
			...
			if(response)
			{
				...
				return ShowPlayerDialog(playerid, DIALOG_GROUP_JURISDICTION_RMV, DIALOG_STYLE_MSGBOX, szTitle, szDialog, "Xac nhan", "Huy bo");
			}
			else return Group_DisplayDialog(playerid, iGroupID);
		}
		case DIALOG_GROUP_JURISDICTION_RMV: {
			new iGroupID = GetPVarInt(playerid, "Group_EditID");
			if(response)
			{
		...
		case DIALOG_GROUP_DISBAND: {
			...
			if(!response && PlayerInfo[playerid][pAdmin] >= 6) {
		...
		case DIALOG_ADPLACEP: {
			if(response) {
				...
				else if(GetPlayerCash(playerid) < 800 && GetPVarInt(playerid, "AdvertVoucher") != 1) {
					ShowPlayerDialog(playerid, DIALOG_ADMAIN, DIALOG_STYLE_LIST, "Quang cao", "Danh sach quang cao\nTim kiem quang cao\nDat quang cao\nDat quang cao uu tien", "Lua chon", "Thoat");
					return SendClientMessageEx(playerid, COLOR_GREY, "Ban khong du tien mat de dat quang cao");
				}
	...
	if(dialogid == PBTOKENBUYMENU)
	{
	    if(response == 1)
	    {
	        ...
	        if(GetPlayerCash(playerid) < 100*strval(inputtext))
	        {
				PaintballTokenBuyMenu(playerid);
	        ...
	        GivePlayerCash(playerid, -100*strval(inputtext));
	    ...
	    else
	    {
	        if( strlen(inputtext) >= 1 )
	        {
	            ...
	            else
	            {
					...
					new namechangecost;
					namechangecost = (PlayerInfo[playerid][pLevel]) * 3000;
		...
		if(response)
		{
			...
			else if(GetPVarInt(playerid, "lockmenu") == 3)
			{
	            ...
	            if (IsNumeric(inputtext))
		        {
					...
					// if(IsABike(PlayerVehicleInfo[playerid][pvid][pvId])) return SendClientMessageEx(playerid, COLOR_GRAD4, "Ban khong the cai dat khoa tren xe dap.");
	...
	if(dialogid == 7483) // VIP Locker /viplocker
	{
		if(response)
		{
			...
			if(listitem == 1)
			{
				...
				if(armour >= 70)
				{
					SendClientMessageEx(playerid, COLOR_GREY, "Ban da co giap.");
					return 1;
				}
				if(PlayerInfo[playerid][pDonateRank] == 1) // VIP DONG
				{
					if(GetPlayerCash(playerid) < 500)
			    	{
			        	SendClientMessageEx(playerid, COLOR_GREY,"Ban khong co du $500!");
			        	return 1;
			    	}
					GivePlayerCash(playerid, -500);
					SetPlayerArmor(playerid, 70);
					SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Ban tra $500 cho 70 giap.");
				}
				else if(PlayerInfo[playerid][pDonateRank] == 2) // VIP BAC
				{
					if(GetPlayerCash(playerid) < 250)
			    	{
			        	SendClientMessageEx(playerid, COLOR_GREY,"Ban khong co du $250!");
			        	return 1;
			    	}
					GivePlayerCash(playerid, -250);
					SetPlayerArmor(playerid, 70);
					SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Ban tra $250 cho 70 giap.");
				}
				if(PlayerInfo[playerid][pDonateRank] >= 3) // VIP VANG
				{
					SetPlayerArmor(playerid, 100);
			...
			if(listitem == 4)
			{
  				if(PlayerInfo[playerid][pDonateRank] >= 3) // 3 la gold tro len 
            	{
			    	ShowPlayerDialog(playerid, 7484, DIALOG_STYLE_LIST, "VIP: Job Center", "Tham tu\nLuat su\nGai diem\nBuon thuoc phien\nVe si\nSua xe\nBuon vu khi\nBoxer\nBuon ma tuy\nTaxi Driver\nTho thu cong\nBartender\nTrucker\nPizza Boy", "Tien hanh", "Huy bo");
		    	}
			    else
			    {
			    	SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Ban phai co Gold VIP tro len moi co the truy cap vao danh sach nhan viec lam.");
			    }
			}
			if(listitem == 5)
			{
			...
			if(GetPlayerCash(playerid) >= 500)
			{
				if(listitem == 0)
				{
    				...
    				if(PlayerInfo[playerid][pDonateRank] >= 1)
				    {
        				...
        				SendClientMessageEx(playerid, COLOR_YELLOW, "VIP: Ban duoc giam gia 20 phan tram khi mua hang. Thay vi phai tra $500, Ban tra $400.");
					}
					else
					{
						GivePlayerCash(playerid, -500);
	...
	if(dialogid == DIALOG_LICENSE_BUY && response) // LICENSE BUY DIALOG -  Mua giay phep
	{
		switch (listitem)
		{
			case 0:
...
/*	if(dialogid == MDC_CHECK && response)
			...
			{ //Desert Eagle\nSPAS-12\nMP5\nM4A1\nAK-47\nSniper Rifle\nShotgun
			    ...
			    */
			    case 0: // CRATE GUNS
				{
				    ...
				    {
						GivePlayerValidWeapon(playerid, 24, 60000); // deagle
				...
				case 1: // CRATE GUNS
				{
				    ...
				    {
						GivePlayerValidWeapon(playerid, 27, 60000); //spas
				...
				case 2: // CRATE GUNS
				{
				    ...
				    {
						GivePlayerValidWeapon(playerid, 29, 60000);//mp5
				...
				case 3: // CRATE GUNS
				{
				    ...
				    {
						GivePlayerValidWeapon(playerid, 31, 60000); //m4
				...
				case 4: // CRATE GUNS
				{
					if(CrateInfo[CrateID][GunQuantity] >= 5)
				    {
						GivePlayerValidWeapon(playerid, 30, 60000); //ak47
						CrateInfo[CrateID][GunQuantity] -= 5;
					}
				}
				case 5: // CRATE GUNS
				{
					if(CrateInfo[CrateID][GunQuantity] >= 5)
				    {
						GivePlayerValidWeapon(playerid, 34, 60000);//sniper
						CrateInfo[CrateID][GunQuantity] -= 5;
					}
				}
				case 6: // CRATE GUNS
				{
				    ...
				    {
						GivePlayerValidWeapon(playerid, 25, 60000);//shotgun
				...
				case 7: // CRATE GUNS
				{
				    ...
				    {
						GivePlayerValidWeapon(playerid, 22, 60000);//shotgun
						CrateInfo[CrateID][GunQuantity] -= 1;
					}
				}
			}
			format(string, sizeof(string), "So Serial: #%d\n So vu khi con lai: %d/50\n (( Duoc dat boi: %s ))", CrateID, CrateInfo[CrateID][GunQuantity], CrateInfo[CrateID][crPlacedBy]);
	        ...
	        switch(listitem)
			{
				case 0: //Vi pham luat
			    {
			        if(PlayerInfo[playerid][pJailTime] > 0) {
			        	...
			        	ShowPlayerDialog(playerid, DIALOG_REPORTDM, DIALOG_STYLE_INPUT, "Bao cao nguoi choi - Vi pham luat", "Nhap ID nguoi choi.", "Dong y", "Huy bo");
					}
				}
			    case 1: //Bug
			    {
			        ...
			        else
			        {
						...
						SetPVarInt(playerid, "_rRepID", playerid);
      					format(Message, sizeof(Message), "{AA3333}AdmWarning{FFFF00}: %s (ID %d) dang bi dung im. (Bug Report)", GetPlayerNameEx(playerid), playerid);
						ABroadCast(COLOR_YELLOW, Message, 2);
						SendReportToQue(playerid, "Bug (Nguoi choi dung im)", 2, GetPlayerPriority(playerid));
						SendClientMessageEx(playerid, COLOR_WHITE, "Bao cao cua ban da duoc gui den Admin.");
			        }
			    }		
			    case 2: //Non RP Name
			    {
			    ...
			    case 3: //Van de ve Shop
	            {
	                ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Van de ve Shop", "{FFFFFF}Ban co the su dung /shophelp de biet them thong tin.", "Dong", "");
	            }		
				case 4: //Yeu cau Unmute
				{
    				...
    				ShowPlayerDialog(playerid, DIALOG_UNMUTE, DIALOG_STYLE_LIST, "Yeu cau Unmute", "Bo khoa quang cao\nBo khoa newbie", "Lua chon", "Thoat");
	            }
	            case 5: //Lien he admin
	            {
	                ShowPlayerDialog(playerid, DIALOG_REPORTNOTLIST, DIALOG_STYLE_INPUT,"Lien he Admin","Dien thong tin ban muon bao cao voi Admin tai day.","Gui","Huy bo");
				}
	...
	else if(dialogid == DIALOG_REPORTNOTLIST)
	{
		...
		{
		    if(isnull(inputtext)) {
		        ShowPlayerDialog(playerid, DIALOG_REPORTNOTLIST, DIALOG_STYLE_INPUT,"Lien he Admin","(Error - Khong co tin nhan) Vui long kiem tra lai thong tin va chac chan rang ban muon gui no","Gui","Huy bo");
		    }

			SetPVarString(playerid, "ReportNotList", inputtext);

			ShowPlayerDialog(playerid, DIALOG_REPORTNOTLIST2, DIALOG_STYLE_MSGBOX, "Lien he Admin", "Vui long kiem tra lai thong tin va chac chan rang ban muon gui no", "Gui", "Huy bo");
		}
	}
	...
	{
		if (PlayerInfo[playerid][pAdmin] >= 6 || PlayerInfo[playerid][pFactionModerator] >= 2)
		{
			new
				iGroupID = listitem,
	...
	else if(dialogid == DIALOG_LOADTRUCKOLD) // TRUCKER JOB LOAD TRUCK
	{
 		if(response)
		{
			...
			if(listitem == 1) // Illegal goods
			{
				...
				if(level >= 0 && level <= 50)
				{
            		ShowPlayerDialog(playerid, DIALOG_LOADTRUCKI, DIALOG_STYLE_LIST, "Ban muon van chuyen gi?","{FF0606}Vu khi {FFFFFF}(Level 1: katana)\n{FF0606}Thuoc phien {FFFFFF}(Level 1: 1 pot, 0 crack)\n{FF0606}Vat lieu bat hop phap {FFFFFF}(Level 1: 2000 vat lieu)", "Lua chon", "Thoat");
				}
				else if(level >= 51 && level <= 100)
				{
		    		ShowPlayerDialog(playerid, DIALOG_LOADTRUCKI, DIALOG_STYLE_LIST, "Ban muon van chuyen gi?","{FF0606}Vu khi {FFFFFF}(Level 2: katana hoac Silenced pistol)\n{FF0606}Thuoc phien {FFFFFF}(Level 2: 1 pot, 1 crack)\n{FF0606}Vat lieu bat hop phap {FFFFFF}(Level 2: 4000 vat lieu)", "Lua chon", "Thoat");
				}
				else if(level >= 101 && level <= 200)
				{
		    		ShowPlayerDialog(playerid, DIALOG_LOADTRUCKI, DIALOG_STYLE_LIST, "Ban muon van chuyen gi?","{FF0606}Vu khi {FFFFFF}(Level 3: 9mm, Silenced pistol hoac Shotgun)\n{FF0606}Thuoc phien {FFFFFF}(Level 3: 2 pot, 1 crack)\n{FF0606}Vat lieu bat hop phap {FFFFFF}(Level 3: 6000 vat lieu)", "Lua chon", "Thoat");
				}
				else if(level >= 201 && level <= 400)
				{
            		ShowPlayerDialog(playerid, DIALOG_LOADTRUCKI, DIALOG_STYLE_LIST, "Ban muon van chuyen gi?","{FF0606}Vu khi {FFFFFF}(Level 4: Shotgun hoac Rifle)\n{FF0606}Thuoc phien {FFFFFF}(Level 4: 2 pot, 2 crack)\n{FF0606}Vat lieu bat hop phap {FFFFFF}(Level 4: 8000 vat lieu)", "Lua chon", "Thoat");
				}
				else if(level >= 401)
				{
 		 			ShowPlayerDialog(playerid, DIALOG_LOADTRUCKI, DIALOG_STYLE_LIST, "Ban muon van chuyen gi?","{FF0606}Vu khi {FFFFFF}(Level 5: Shotgun, Rifle hoac Deagle (hiem)\n{FF0606}Thuoc phien {FFFFFF}(Level 5: 3 pot, 3 crack)\n{FF0606}Vat lieu bat hop phap {FFFFFF}(Level 5: 10000 vat lieu)", "Lua chon", "Thoat");
				}
			}
		}
		else
	...
	else if(dialogid == DIALOG_LOADTRUCKI) // TRUCKER JOB ILLEGAL GOODS
	{
 		if(response)
		{
			...
			if(listitem == 0) // Weapons
			{
				...
				/*if(level >= 0 && level <= 50)
				{
                    SetPVarInt(playerid, "TruckDeliver", 11); // Bonus: Katana
				}
				else if(level >= 51 && level <= 100)
				{
                    SetPVarInt(playerid, "TruckDeliver", 12); // Bonus: Katana va sdpistol
				}
				else if(level >= 101 && level <= 200)
				{
                    SetPVarInt(playerid, "TruckDeliver", 13); // Bonus: 9mm, sdpistol va shotgun
				}
				else if(level >= 201 && level <= 400)
				{
                    SetPVarInt(playerid, "TruckDeliver", 14); // Bonus: shotgun va rifle
				}
				else if(level >= 401)
				{
                    SetPVarInt(playerid, "TruckDeliver", 15); // Bonus: shotgun, rifle va deagle
				}*/
			...
			if(listitem == 1) // Drugs
			{
				...
				/*if(level >= 0 && level <= 50)
				{
                    SetPVarInt(playerid, "TruckDeliver", 16); // Bonus: 1 pot, 0 crack
				}
				else if(level >= 51 && level <= 100)
				{
                    SetPVarInt(playerid, "TruckDeliver", 17); // Bonus: 1 pot, 1 crack
				}
				else if(level >= 101 && level <= 200)
				{
                    SetPVarInt(playerid, "TruckDeliver", 18); // Bonus: 2 pot, 1 crack
				}
				else if(level >= 201 && level <= 400)
				{
                    SetPVarInt(playerid, "TruckDeliver", 19); // Bonus: 2 pot, 2 crack
				}
				else if(level >= 401)
				{
                    SetPVarInt(playerid, "TruckDeliver", 20); // Bonus: 3 pot, 3 crack
				}*/
			...
			if(listitem == 2) // Illegal materials
			{
				...
				{
                    SetPVarInt(playerid, "TruckDeliver", 22); // Bonus:  materials
				}
				else if(level >= 101 && level <= 200)
				{
                    SetPVarInt(playerid, "TruckDeliver", 23); // Bonus: 100 materials
				}
				else if(level >= 201 && level <= 400)
				{
                    SetPVarInt(playerid, "TruckDeliver", 24); // Bonus: 150 materials
				}
				else if(level >= 401)
				{
                    SetPVarInt(playerid, "TruckDeliver", 25); // Bonus: 200 materials
				}*/
	...
	if(dialogid == DIALOG_SHOPHELPMENU)
	{
	    if(response)
	    {
	    	switch(listitem)
	    	{
	    	    ...
	    	    case 1:
	    	    {
	    	        ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Shop nha", "De mua nha, thay doi noi that, thay doi vi tri nha, ban co the su dung /houseshop.", "Thoat", "");
	    	    }
	    	    case 2:
	    	    {
	    	        ShowPlayerDialog(playerid, DIALOG_NOTHING, DIALOG_STYLE_MSGBOX, "Shop cua hang", "Ban muon mua cua hang hoac gia han them cua hang cua ban? Du dung lenh /businessshop se cho phep ban mua hoac gia han them cua hang cua ban.\n Dieu quan trong khi kinh doanh tai GVN ban phai doc nhung quy dinh tren dien dan forum.GTA-Viet.net Chu y: Khi mua cua hang se tu dong liet ke cac cua hang dang bay ban.", "Thoat", "");
	    	    }
	    	    case 3:
	    	    {
	...
	if(dialogid == DIALOG_VOUCHER)
	{
		if(response)
		{
			...
			{
				if(PlayerInfo[playerid][pAdmin] >= 6 || PlayerInfo[playerid][pHR] >= 1)
				{
	...
	if(dialogid == DIALOG_CONFIRMADP)
	{
		if(response)
		{
			...
			format(advert, sizeof(advert), "[QC]: %s - Lien he: %s (%d)", advert, GetPlayerNameEx(reportid), PlayerInfo[reportid][pPnumber]);
