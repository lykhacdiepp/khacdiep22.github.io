...
public OnPlayerUpdate(playerid)
{
    if(SweeperJob[playerid] && GetVehicleModel(GetPlayerVehicleID(playerid)) == 574 && SweeperUpdate[playerid] < tickcount())
    {
        ...
        new string[64];
        format(string, sizeof(string), "~b~~h~Nhan vien ve sinh~n~~n~~w~Don sach: ~y~%d Met", SweeperDistance[playerid]);
...
{
    if(newstate == PLAYER_STATE_DRIVER && GetVehicleModel(GetPlayerVehicleID(playerid)) == 574 && !SweeperJob[playerid]) GameTextForPlayer(playerid, "~n~~n~~w~Bam so ~y~~k~~TOGGLE_SUBMISSIONS~ ~w~de lam cong viec~n~~b~~h~~h~nhan vien ve sinh", 3000, 3);
    if(oldstate == PLAYER_STATE_DRIVER && SweeperJob[playerid])
    {
        ...
        format(string, sizeof(string), "~n~~n~~w~Ban da don sach: ~b~~h~~h~%d Met~n~~w~So tien kiem duoc ~g~~h~~h~$%d", SweeperDistance[playerid], money);
...
{
    if(PRESSED(KEY_YES) && GetVehicleModel(GetPlayerVehicleID(playerid)) == 574 && !SweeperJob[playerid])
    {
        SweeperText[playerid] = CreatePlayerTextDraw(playerid, 40.000000, 305.000000, "~b~~h~Nhan vien ve sinh~n~~n~~w~Don sach: ~y~0 Met");
        ...
        SendClientMessage(playerid, -1, "Ban hay lai xe xung quanh de don dep thanh pho!");
        SendClientMessage(playerid, -1, "Ban se duoc nhan tien luong khi roi khoi xe sweeper.");
    }
